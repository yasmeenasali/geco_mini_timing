...
# (c) Stefan Countryman 2017, updated with new channel names Yasmeen Asali 2019 
# several functions translated from MATLAB code by
...
# Currently (3/4/19) DAC channels seem to be unconnected 

DESC="""A module (that can also be used as a script) for plotting delay
histograms in DuoTone signals. Adapted version of Stefan Countryman's code. Code is written
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t','--gpstime', type=float,
                        help=('GPS time of the event with nanoseconds.'))
    parser.add_argument('--sid', type=str,
                        help=('Superevent ID of the event.'))
...
def commissioningFrameDuotoneStat(IFO, full_time, superevent, title=False):
    """Directly translated from Keita's MATLAB function of the same name.
    Docstring copied from MATLAB function.
    ...
    if title:
        plt.suptitle('{} Event Time: {}'.format(superevent, full_time))
    plt.savefig('duotone_stat_plots_{}_{}.pdf'.format(IFO, int(eGPS)), bbox_inches='tight' )
...
if __name__ == "__main__":
    # make sure gpstime included
    if args.gpstime is None or args.sid is None:
        print('ERROR: Must provide gpstime and superevent ID of event.\n')
        print(DESC)
        exit(1)
    print('Making commissioningFrameDuotoneStat plots.')
    commissioningFrameDuotoneStat('H1', args.gpstime, args.sid, title=False) #TODO reformat subplots to add title
    commissioningFrameDuotoneStat('L1', args.gpstime, args.sid)
